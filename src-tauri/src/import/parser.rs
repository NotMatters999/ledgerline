...
fn parse_excel(path: &Path) -> Result<ParsedFile, ParserError> {
    ...
    let headers = match iter.next() {
        Some(row) => row.iter().map(|cell| match cell {
            ...
            Data::DateTime(d) => {
                if *d >= 60.0 {
                    chrono::NaiveDate::from_ymd_opt(1899, 12, 30).and_then(|base| base.checked_add_signed(chrono::Duration::days(*d as i64))).map(|dt| dt.format("%Y-%m-%d").to_string()).unwrap_or_else(|| d.to_string())
                } else if *d > 0.0 {
                    chrono::NaiveDate::from_ymd_opt(1899, 12, 31).and_then(|base| base.checked_add_signed(chrono::Duration::days(*d as i64))).map(|dt| dt.format("%Y-%m-%d").to_string()).unwrap_or_else(|| d.to_string())
                } else {
                    d.to_string()
                }
            },
