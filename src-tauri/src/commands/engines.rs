...
#[tauri::command]
pub fn mrr_get(workspace_id: String, state: State<'_, AppState>) -> Result<Vec<MrrMovement>, String> {
    let conn = get_workspace_conn(&workspace_id, &state)?;
    calculate_mrr(&conn).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn arr_get(workspace_id: String, state: State<'_, AppState>) -> Result<Vec<ArrMovement>, String> {
    let conn = get_workspace_conn(&workspace_id, &state)?;
    calculate_arr(&conn).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn retention_get(workspace_id: String, state: State<'_, AppState>) -> Result<Vec<RetentionMovement>, String> {
    let conn = get_workspace_conn(&workspace_id, &state)?;
    calculate_retention(&conn).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn ltv_get(workspace_id: String, state: State<'_, AppState>) -> Result<Vec<LtvMovement>, String> {
    let conn = get_workspace_conn(&workspace_id, &state)?;
    calculate_ltv(&conn).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn cac_get(workspace_id: String, state: State<'_, AppState>) -> Result<Vec<CacMovement>, String> {
    let conn = get_workspace_conn(&workspace_id, &state)?;
    calculate_cac(&conn).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn payback_get(workspace_id: String, state: State<'_, AppState>) -> Result<Vec<PaybackMovement>, String> {
    let conn = get_workspace_conn(&workspace_id, &state)?;
    calculate_payback(&conn).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn forecast_get(workspace_id: String, params: ForecastParams, state: State<'_, AppState>) -> Result<Vec<ForecastMovement>, String> {
    let conn = get_workspace_conn(&workspace_id, &state)?;
    calculate_forecast(&conn, &params).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn cohort_get(workspace_id: String, state: State<'_, AppState>) -> Result<CohortData, String> {
    let conn = get_workspace_conn(&workspace_id, &state)?;
